...
export default {
    ...
    mounted(){
        ...
        function Ball(x, y, defaultRadius) {
            ...
            this.path = function()  {
                ...
                while(Math.pow((Math.pow(this.x-currentPos[0], 2) + Math.pow(this.y-currentPos[1],2)),0.5) > 1 || iterations < 100){
                    ...
                    if(currentPos[0] < (canvas.width - canvas.width*this.scale) || currentPos[0] > canvas.width*this.scale || currentPos[1] < (canvas.height - canvas.height*this.scale) || currentPos[1] > canvas.height*this.scale){
                    ...
                    currentPos[0] += currentVel[0] * stepSize;
                    currentPos[1] += currentVel[1] * stepSize;
                    ...
                    for(let i = 0; i < massCentres.length; i++){
                        ...
                        currentVel[0] = currentVel[0] + massCentres[i][2]*Math.abs(Math.pow(dist , -3))*(massCentres[i][0]-currentPos[0]);
                        currentVel[1] = currentVel[1] + massCentres[i][2]*Math.abs(Math.pow(dist , -3))*(massCentres[i][1]-currentPos[1]);
                    }

                    this.pathCoords.push(currentPos.slice());
                    iterations += 1;

                    if(iterations > 10000){this.closedPath = false; break; }
                }
        ...
        function animate() {
            ...
            for (let i = 0; i < ballArray.length; i++){    
                c.clearRect((canvas.width-canvas.width*ballArray[i].scale),(canvas.height-canvas.height*ballArray[i].scale), canvas.width*ballArray[i].scale*2, canvas.height*ballArray[i].scale*2);
            }
